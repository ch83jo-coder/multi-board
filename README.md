...
`OPENAI_API_KEY`を設定すると、実測データ登録画面で写真からのAI自動入力を利用できます。画像は入力補助のためOpenAI APIへ送信され、Panmoaのストレージには保存されません。Vercelへデプロイする場合は、同じキーをVercelのEnvironment Variablesにも登録してください。
