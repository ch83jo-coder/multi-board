...
Web全体から検索して確認した最近の話題をもとに、日本語のコミュニティ投稿を手動生成する管理用ツールです。Tesla公式サイトなど特定ドメインには限定せず、日本向けの一般的なWeb検索として政府・自治体、充電事業者、保険会社、自動車関連企業、報道機関、専門メディアなども検索対象にします。既存の初期データ投入ツールとは独立しており、有効な掲示板を選んで実行します。

検索にはGoogle Search APIではなく、OpenAI Responses APIのWeb Searchを使用します。Googleの一般検索と同様に検索対象をWeb全体へ広げる構成で、`allowed_domains`によるドメイン制限は設定していません。
...
- 各投稿は特定ドメインに限定しないWeb検索で複数の運営元を確認し、話題を独自に要約・コメントして、最終行に`参考: https://...`形式の情報源を付けます。
