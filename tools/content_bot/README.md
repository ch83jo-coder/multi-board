...
Web全体から検索して確認した最近の話題をもとに、日本語のコミュニティ投稿を手動生成する管理用ツールです。話題の範囲は対象掲示板の名称と説明から決まり、ツール側で特定のテーマを固定していません。特定ドメインには限定せず、日本向けの一般的なWeb検索として政府・自治体、業界団体、関連企業、報道機関、専門メディアなども検索対象にします。既存の初期データ投入ツールとは独立しており、有効な掲示板を選んで実行します。
...
`SUPABASE_SERVICE_ROLE_KEY`と`OPENAI_API_KEY`は秘密鍵です。ブラウザへ公開したり、Gitへコミットしたりしないでください。`--author`と`--guest-name`を省略した場合は、`profiles.role = 'admin'`の最も古い管理者プロフィールが投稿者になります。一般会員を指定する場合は、対象プロフィールの`role`が`member`である必要があります。

## 実行方法

引数を付けずに実行すると、次の項目を対話形式で順番に選択できます。

1. 投稿先の掲示板、またはすべての掲示板
2. 掲示板ごとの投稿数（1〜10件）
3. 投稿画像を生成するか
4. ゲスト投稿者名（任意のランダム名、空欄なら管理者）
5. 実行内容の最終確認

最終確認で`y`を入力した場合だけ生成と保存を開始します。`n`またはEnterを入力すると安全にキャンセルします。

```bash
yarn content-bot
```

引数を使う従来の非対話形式も利用できます。1つの掲示板へ既定の5件を生成します。
...
特定の一般会員名義で投稿する場合は、ユーザー名またはUUIDを指定します。管理者プロフィールは`--author`では指定できません。

```bash
yarn content-bot --board news --count 3 --author member_name
```

プロフィールに存在しない任意のランダム名で投稿する場合は`--guest-name`を指定します。名前は1〜30文字で、アカウントやプロフィールは作成されません。指定した名前が、その実行で作成するすべての投稿に`guest_name`として保存されます。

```bash
yarn content-bot --all --guest-name "充電初心者"
```

`--author`と`--guest-name`は同時に指定できません。
...
標準入力または標準出力がTTYではないCI・自動化環境で引数を省略した場合は、入力待ちにならず有効な掲示板と使用方法を表示して終了します。

## 生成・保存ルール

- 話題の範囲は`boards.name`と`boards.description`をプロンプトへ渡して指示します。掲示板の説明が具体的なほど話題が絞り込まれます。
- 掲示板ごとに選択された投稿者名義の最近の投稿タイトルを最大30件取得し、プロンプトへ渡して話題の重複を抑えます。
- 保存直前にも掲示板内の完全一致タイトルを確認し、同一タイトルはスキップします。
- 各投稿は特定ドメインに限定しないWeb検索で複数の運営元を確認し、話題を独自に要約・コメントします。情報源は生成JSONの`source_url`として受け取り、ツール側が最終行に`参考: https://...`形式で追加します。
- 記事本文の転載、長い引用、センシティブな事件は生成対象から除外するよう指示しています。
- 投稿は通常の初期値で保存します。閲覧数や投票数の水増し、コメント生成は行いません。
- `--with-images`では投稿ごとに独自の横長ビジュアルを生成し、既存の公開Supabase Storageバケット`post-images`へWebP形式でアップロードします。最大10件の各投稿には、コラージュ、3Dジオラマ、手描き、テクニカル断面、ベクター、クレイ、写真、スティルライフ、リソグラフ、抽象ガラスの異なる表現を重複なしで割り当てます。公開URLは`posts.thumbnail_url`へ保存します。
